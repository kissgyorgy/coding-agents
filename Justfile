...
build *args:
    #!/usr/bin/env bash
    set -euo pipefail
    all="claude-code claude-code-ui gemini-cli ccusage codex crush pi-coding-agent llmfit llmserve"
    attrs=""
...
update:
    ...
    set -uo pipefail
    failed=()
    for pkg in claude-code codex gemini-cli crush pi-coding-agent llmfit llmserve; do
        echo "==> Updating $pkg"
